...
import {
  getTeacherSchedules,
  getEnrolledStudentsByScheduleAndTeacher,
} from "../api";
...
export default function MySchedules(props) {
  ...
  const columns = [
    {
      title: "Subject",
      render: (val) => <span>{val.subject}</span>,
    },
    {
      title: "Year Level",
      render: (val) => <span> {val.year_level}</span>,
    },
    {
      title: "Days",
      render: (val) => <span>{DisplayDate(val)}</span>,
    },
    {
      title: "Time",
      render: (val) => <span>{DisplayTime(val)}</span>,
    },
    {
  ...
  function DisplaySelected(csT) {
    try {
      return (
        <b>
          Enrolled Students for {csT.subject}
          <br /> {csT.date} | {csT.time}{" "}
        </b>
      );
    } catch (err) {
      console.log(err.message);
  ...
  const studentCol = [
    {
      ...
      render: (val) => (
        <Row>
          ...
          <Col xs={24} lg={17}>
            <span>
              <b>Name: </b> {val.student.name}
            </span>{" "}
            <br />
            <span>
              <b>Birthdate:</b>{" "}
              {moment(val.student.birthdate).format("MM-DD-YYYY")}
            </span>
          </Col>
        </Row>
      ),
