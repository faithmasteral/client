...
export default function MyClass(props) {
  ...
  const columns = [
    {
      title: "Subject",
      render: (val) => <span>{val.class_schedule.subject}</span>,
    },
    {
      title: "Year Level",
      render: (val) => <span> {val.class_schedule.year_level}</span>,
    },
    {
      title: "Days",
      render: (val) => <span>{DisplayDate(val.class_schedule)}</span>,
    },
    {
      title: "Time",
      render: (val) => <span>{DisplayTime(val.class_schedule)}</span>,
    },
    {
      title: <center>Teacher</center>,
      render: (val) => <center>{val.teacher.name}</center>,
    },
  ];
  ...
  function DisplaySched(cs) {
    try {
      ...
      console.log(dayNow);
      return (
        <Row gutter={[24, 5]} style={{ marginBottom: 10 }}>
          {cs !== null &&
            cs.map((val, k) => {
              {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Row>
                      <Col xs={24} xl={12}>
                        <b>{val.class_schedule.subject}</b>
                        <br />
                      ...
                      <Col xs={24} xl={12}>
                        ...
                        <small style={{ float: "right" }}>
                          {" "}
                          {DisplayDate(val.class_schedule)}
                        </small>
                        <br />
                      </Col>
                    </Row>
